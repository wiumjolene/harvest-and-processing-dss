...
SELECT f_from_to.id, dim_packhouse.name as site, dim_fc.name as grower, dim_fc.name as block, dim_fc.long_name as grower,
...
LEFT JOIN dim_fc ON (dim_fc.id=f_from_to.fc_id)
...
LEFT JOIN dim_packhouse ON (dim_packhouse.id=f_from_to.packhouse_id)
LEFT JOIN dim_fc ON (dim_fc.id=f_from_to.fc_id)
LEFT JOIN (SELECT *, 1 as 'yes' FROM dss.history_from_to WHERE perc > 0.04999) historic 
	ON (historic.grower=dim_fc.name AND historic.packsite=dim_packhouse.name)
SET `history` = '1' 
WHERE (historic.yes = 1); */


UPDATE `dss`.`f_from_to` SET `allowed` = '1' 
where history=1; 