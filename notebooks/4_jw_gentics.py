...
fitness_df = pd.read_excel(path + '\\data\\interim\\fitness.xlsx')

def tournament_selection(fitness_df):
    ...
    parent_path = f"{path}\data\interim\id_{parent}.xlsx" #TODO: Make pickle
...
#def crossover(fitness_df):
""" GA crossover genetic material for diversivication"""
ix = Individual()
max_id = fitness_df.id.max()

ddf_metadata = pd.read_pickle(path + '\\data\\processed\\ddf_metadata') # FIXME: update path
